...
resource "restapi_object" "company_apps" {
  ...
  data = "{ \"connector_id\": \"${each.value.connector_id}\", \"name\":\"${each.value.name}\", \"policy_id\": \"${var.ol_baseline_app_policy_id}\", \"visible\":\"${each.value.visible}\", \"description\":\"Birthright-${each.value.birthright_app}\", \"notes\":\"${each.value.notes}\", \"allow_assumed_signin\":\"${each.value.allow_assumed_signin}\"}"
}
...
resource "restapi_object" "company_apps2" {
  ...
  data = "{ \"connector_id\": \"${each.value.connector_id}\", \"name\":\"${each.value.name}\", \"policy_id\": \"${each.value.policy_id}\", \"visible\":\"${each.value.visible}\", \"description\":\"Birthright-${each.value.birthright_app}\", \"notes\":\"${each.value.notes}\", \"allow_assumed_signin\":\"${each.value.allow_assumed_signin}\", \"tab_id\":\"${each.value.tab_id}\"}"
}
