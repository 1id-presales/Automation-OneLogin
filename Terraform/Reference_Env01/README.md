...
The solution will also see the creation of 3 Application Entitlements OneLogin Roles for each application. These roles can then be used in the application connector configuration by the app owner when configuring the connector to control how and when application rules may or may not trigger based on the membership status of these Application Entitlements roles. For example the Application Entitlements roles could be used to assign different licenses to different users when they are provisioned into the target application.
