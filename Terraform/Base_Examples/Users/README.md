...
- Run a terraform plan operation to see what changes will be applied to your environment. Now there will be one planned change to the existing resource **onelogin_users.username** If you are happy with this run the command **terraform plan -var-file "target_ol_env.tfvars"** to update your OneLogin environment
- If you are happy with the output of the plan from the previous step proceed to apply the planned changes. Apply the planned changes with the command **terraform apply -var-file "target_ol_env.tfvars"**
